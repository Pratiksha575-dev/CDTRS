...
class DashboardPage(QWidget):
    ...
    def refresh(self):
        ...
        if self.role == RoleEnum.DIRECTOR.value:
            ...
            reviewed_count = sum(1 for d in self.documents if d.director_remark is not None and d.current_stage != "DIRECTOR")
            ...
            pending_count = sum(1 for d in emp_docs if len(progress_service.get_progress_updates(d.id) if d.id else []) == 0)
            progress_count = sum(1 for d in emp_docs if len(progress_service.get_progress_updates(d.id) if d.id else []) > 0)
            critical_count = sum(1 for d in emp_docs if (d.priority or "").lower() in ("high", "red", "orange"))
            ...
            if hasattr(self, "act_card_intake"):
                ...
                self.act_card_deadlines["sub_label"].setText(f"{len(self.documents)} registered documents in system")
