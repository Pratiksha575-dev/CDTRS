...
try:
    from OCR.ocr import DocumentOCR as _DocumentOCR   # OCR/ocr.py
    _OCR_AVAILABLE = True
except ImportError:
    try:
...
class OCRService:
    ...
    def process_incoming_document(
        ...
        print(f"* Extraction Engine : {'PaddleOCR-v3 (Neural)' if _OCR_AVAILABLE else 'Digital Text / PyMuPDF Extractor'}", flush=True)
