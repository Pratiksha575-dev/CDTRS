...
class MockRepository(BaseRepository):
    ...
    def _seed_users(self):
        ...
        self._users: Dict[str, UserModel] = {
            ...
            "ds_user": UserModel(id=1, username="ds_user", full_name="Director Secretary", role=RoleEnum.DIRECTOR_SECRETARY.value),
            "director": UserModel(id=2, username="director", full_name="The Director", role=RoleEnum.DIRECTOR.value),

            # Department 1: Finance (HOD ID: 3, Staff: Rahul Sharma, Sunil Gupta)
            "hod_finance": UserModel(id=3, username="hod_finance", full_name="Head of Finance", role=RoleEnum.HOD.value, department_id=1, department_name="Finance"),
            ...
            "emp_sunil": UserModel(id=102, username="emp_sunil", full_name="Sunil Gupta", role=RoleEnum.EMPLOYEE.value, department_id=1, department_name="Finance"),

            # Department 2: HR (HOD ID: 4, Staff: Sneha Deshmukh, Pooja Nair)
            "hod_hr": UserModel(id=4, username="hod_hr", full_name="Head of Human Resources", role=RoleEnum.HOD.value, department_id=2, department_name="HR"),
            "emp_sneha": UserModel(id=201, username="emp_sneha", full_name="Sneha Deshmukh", role=RoleEnum.EMPLOYEE.value, department_id=2, department_name="HR"),
            "emp_pooja": UserModel(id=202, username="emp_pooja", full_name="Pooja Nair", role=RoleEnum.EMPLOYEE.value, department_id=2, department_name="HR"),

            # Department 3: Technical (HOD ID: 5, Staff: Anil Kumar, Vikram Malhotra)
            "hod_tech": UserModel(id=5, username="hod_tech", full_name="Head of Technical & IT", role=RoleEnum.HOD.value, department_id=3, department_name="Technical"),
            "emp_anil": UserModel(id=301, username="emp_anil", full_name="Anil Kumar", role=RoleEnum.EMPLOYEE.value, department_id=3, department_name="Technical"),
            "emp_vikram": UserModel(id=302, username="emp_vikram", full_name="Vikram Malhotra", role=RoleEnum.EMPLOYEE.value, department_id=3, department_name="Technical"),
        }

        # Set default active session to DS for initial development
        self._current_user = self._users["ds_user"]
    ...
    def _seed_documents(self):
        ...
        self._documents: List[DocumentModel] = [
            ...
            DocumentModel(
                ...
                file_type="PDF",
                format="PDF",
                ocr_text="Ministry of Higher Education official directive regarding governance compliance, institutional accreditation standards, and regulatory human resources reporting for the current academic session.",
                attachment_count=1,
            ...
            DocumentModel(
                ...
                file_type="PDF",
                format="PDF",
                ocr_text="State Audit Bureau notification regarding Q3 financial audit, expenditure compliance, ledger reconciliation, and capital grant fund disbursement schedule.",
                attachment_count=2,
            ...
            DocumentModel(
                ...
                file_type="PDF",
                format="PDF",
                ocr_text="Central Board of Direct Taxes statutory vendor tax clearance and procurement verification report. Case assigned to Rahul Sharma for tax ledger verification.",
                attachment_count=1,
            ...
            DocumentModel(
                ...
                format="PDF",
                ocr_text="Office of the Director approval order for campus security infrastructure upgrade. Directive: Approved. Expedite procurement and assign to Rahul Sharma for immediate execution.",
                attachment_count=1,
            ...
            DocumentModel(
                ...
                file_type="PDF",
                format="PDF",
                ocr_text="Cyber Security Directorate compliance notification regarding enterprise server maintenance, core firewall rules validation, and infrastructure uptime SLAs.",
                attachment_count=2,
    ...
    def get_ocr_result(self, document_id: int) -> Dict[str, Any]:
        """Mock: return OCR text, confidence, and status for document."""
        doc = self.get_document(document_id)
        ocr_txt = getattr(doc, "ocr_text", "") or getattr(doc, "description", "") or ""
        if doc and (ocr_txt or doc.title):
            extracted = ocr_txt or doc.title
            return {
                "document_id":    document_id,
                ...
                "extracted_fields": [
                    {"field_name": "TITLE", "extracted_value": doc.title, "confidence": 0.95},
                    {"field_name": "REFERENCE_NO", "extracted_value": doc.reference_no or f"CDTRS-2026-{doc.id:04d}", "confidence": 0.95},
                    {"field_name": "DEPARTMENT", "extracted_value": doc.suggested_department_name or doc.target_department_name or "", "confidence": 0.95},
                ],
            }
        return {
            "document_id":    document_id,
    ...
    def trigger_ocr(self, document_id: int) -> Dict[str, Any]:
        """Mock: acknowledge client-side OCR completion."""
        return {"message": "OCR processed and indexed.", "ocr_status": "COMPLETED"}

    def get_routing_suggestion(self, document_id: int) -> Dict[str, Any]:
        """Mock: return confirmed intake routing suggestion for document."""
        doc = self.get_document(document_id)
        if not doc:
            return {}
        sugg_dept = doc.suggested_department_name or doc.target_department_name
        sugg_emp = doc.suggested_employee_name or doc.assigned_employee_name
        if sugg_dept or sugg_emp:
            return {
                "document_id": document_id,
                "suggested_department_name": sugg_dept,
                "suggested_department_id": doc.suggested_department_id or doc.target_department_id,
                "suggested_employee_name": sugg_emp,
                "suggested_employee_id": doc.suggested_employee_id or doc.assigned_employee_id,
                "routing_confidence": 0.95,
                "routing_reason": "Routing intelligence confirmed during document intake verification.",
                "is_director_instruction": bool(doc.has_prior_director_remark or doc.director_remark),
            }
        return {}

    def analyze_routing(self, document_id: int) -> Dict[str, Any]:
        """Mock: return current routing suggestion."""
        return self.get_routing_suggestion(document_id)
