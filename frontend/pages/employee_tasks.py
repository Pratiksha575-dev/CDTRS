    ...
    def load_tasks(self):
        ...
        if emp_id is None:
            ...
            if not self.documents:
                curr_u = auth_service.get_current_user()
                if curr_u and (curr_u.role or "").lower() == "employee":
                    self.documents = [
                        d for d in all_docs
                        if d.current_stage in (WorkflowStageEnum.EMPLOYEE.value, WorkflowStageEnum.CLOSED.value, "EMPLOYEE", "Employee", "Closed")
                    ]
