...
class HistoryPage(QWidget):
    ...
    def __init__(self):
        super().__init__()
        self._all_docs: List[DocumentModel] = []
        self._all_events: List[WorkflowEventModel] = []
    ...
    def load_history(self):
        """Fetches latest documents and audit events from repository, then filters in-memory."""
        try:
            self._all_docs = document_service.get_documents() or []
        except Exception:
            self._all_docs = []

        try:
            self._all_events = workflow_service.get_all_audit_history() or []
        except Exception:
            self._all_events = []
    ...
    def apply_filters(self):
        """In-memory filtering of cached audit events without triggering redundant network calls."""
