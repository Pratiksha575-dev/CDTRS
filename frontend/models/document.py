...
class DocumentModel:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "DocumentModel":
        ...
        return cls(
            ...
            routing_instruction_confidence=int(data.get("routing_instruction_confidence") or round((data.get("routing_confidence") or data.get("confidence") or 0.95) * 100) if (data.get("routing_confidence") and data.get("routing_confidence") <= 1.0) else (data.get("routing_confidence") or 0)),
            routing_confidence=float(data.get("routing_confidence")) if data.get("routing_confidence") is not None else (float(data.get("confidence")) if data.get("confidence") is not None else None),
            confidence=float(data.get("confidence")) if data.get("confidence") is not None else (float(data.get("routing_confidence")) if data.get("routing_confidence") is not None else None),
