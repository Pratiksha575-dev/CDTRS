...
class DocumentViewer(QWidget):
        ...
        try:
            ...
            if progress_entries:
                ...
                for p in progress_entries:
                    ...
                    p_hdr = QLabel(f"Submitted by {p.user_name or 'Employee'} • {p.created_at or 'Recent'}")
    ...
    def _render_ds_actions(self):
        ...
        if stage == WorkflowStageEnum.DS.value:
            ...
            fwd_btn = QPushButton("Forward Follow-up to Director")
    ...
    def _director_return_to_ds(self):
        ...
        try:
            ...
            if remark_text and remark_text != self.document.director_remark:
                routing_service.save_director_remark(self.document.id or 0, remark=remark_text)

            doc_id = self.document.id or 0
            updated_doc = routing_service.return_to_ds(doc_id, remarks=None)
        ...
        try:
            ...
            if getattr(self.document, "suggested_employee_id", None) or getattr(self.document, "suggested_employee_name", None):
                ...
                QMessageBox.information(
                    self,
                    "Routing Confirmed",
                    f"Document {self.document.reference} routed directly to {updated_doc.assigned_employee_name}."
                )
            elif getattr(self.document, "suggested_department_id", None) or getattr(self.document, "suggested_department_name", None):
                dept_id = getattr(self.document, "suggested_department_id", None) or (1 if "Finance" in (getattr(self.document, "suggested_department_name", "") or "") else 2)
