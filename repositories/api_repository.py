...
class APIRepository(BaseRepository):
    ...
    def get_users(self, role: Optional[str] = None, department_id: Optional[int] = None) -> List[UserModel]:
        """
        Retrieves users. Uses /users for DS role and /departments/{id}/employees
        where role scoping restricts /users access.
        """
        curr_role = (self._current_user.role or "").lower() if self._current_user else ""
        is_ds = curr_role in ("director secretary", "master", "ds")
        ...
        if is_ds:
            ...
            try:
                data = api_client.get(Endpoints.DEPARTMENT_EMPLOYEES(department_id))
                users = [
                    UserModel(
                ...
                users = []
        elif self._current_user and self._current_user.department_id:
            try:
                data = api_client.get(Endpoints.DEPARTMENT_EMPLOYEES(self._current_user.department_id))
                users = [
                    UserModel(
                        id=emp.get("user_id") or emp.get("id"),
                        username=emp.get("employee_code", f"emp_{emp.get('id')}"),
    ...
    def get_documents(
        ...
        """
        Retrieves documents list. Uses /documents for DS and /documents/inbox for non-DS roles.
