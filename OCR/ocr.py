...
def _get_ocr(handwriting: bool = False) -> Any:
    ...
    if handwriting:
        if _paddle_hw is None:
            # Strip internal-only config keys before passing to PaddleOCR
            cfg = {
                k: v for k, v in HANDWRITING_CONFIG.items()
                if not k.startswith("preprocess_")
            }
            # Map v2 key names → v3 key names
...
def _map_config_to_v3(cfg: dict) -> dict:
    ...
    for k, v in cfg.items():
        ...
        if new_k in ACCEPTED:
            out[new_k] = v
    return out


# ===========================================================================
# Internal: parse PaddleOCR v3 OCRResult
# ===========================================================================

def _parse_ocr_result(result: Any) -> tuple[list[str], list[float]]:
    """
    Extract (texts, scores) from a PaddleOCR v3 OCRResult object.

    The OCRResult is a dict-like object with keys:
        'rec_texts'  : list[str]
